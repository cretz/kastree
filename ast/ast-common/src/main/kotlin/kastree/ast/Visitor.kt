...
open class Visitor {
    open fun <T: Node?> T.visit(parent: Node) = when (this) {
        is Node.File -> {
            visitChildren(anns)
            visitChildren(pkg)
    ...
    inline fun <T: Node?> Node?.visitChildren(v: T) { v.visit(this!!) }

    open fun <T: Node?> Node?.visitChildren(v: List<T>) { v.forEach { orig -> orig.visit(this!!) } }
}